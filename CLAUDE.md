...
- **Correção (2026-07-13): a Apple abriu o iOS no Brasil para marketplaces alternativos em 18/06/2026 — a AltStore PAL já está disponível no país** (fonte: usuário + matéria do MacMagazine). A suposição anterior de que PAL era exclusiva da UE estava desatualizada (corte de conhecimento é jan/2026). Requisitos: Conta Apple brasileira, fisicamente no país, iOS 26.5+; instalação via Safari em `https://altstore.io/download`. O usuário está migrando de AltStore Classic (com AltServer no Windows) para PAL (instala/atualiza OTA, sem AltServer) — aguardando confirmação de que o `pages/apps.json` já publicado funciona como source na PAL sem mudanças. Enquanto isso, as sources (deste repo e do photovault) seguem registradas e funcionais na Classic.
